    ...
    <main style={{ minHeight: '100vh', background: '#020608', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'sans-serif' }}>
      <div style={{ width: '100%', maxWidth: '420px', padding: '20px' }}>
        <div style={{ background: '#0d1a20', borderRadius: '20px', padding: '40px', border: '1px solid rgba(0,200,255,.1)' }}>
          ...
          <p style={{ color: '#6a8a95', fontSize: '14px', textAlign: 'center', marginBottom: '32px' }}>Nueva contraseña</p>
          ...
          {done ? (
            <div style={{ background: 'rgba(0,200,100,.1)', border: '1px solid rgba(0,200,100,.2)', borderRadius: '10px', padding: '16px', color: '#00c864', fontSize: '14px', textAlign: 'center' }}>
              <p style={{ marginBottom: '8px', fontWeight: 600 }}>✓ Contraseña actualizada</p>
              <a href="/login" style={{ color: '#00c8ff', fontSize: '13px', textDecoration: 'none' }}>Iniciar sesión</a>
            </div>
          ) : (
            <>
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div>
                  <label style={{ fontSize: '12px', color: '#6a8a95', fontWeight: 600, display: 'block', marginBottom: '6px' }}>NUEVA CONTRASEÑA</label>
                  <input type="password" required placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} style={{ width: '100%', background: '#111e25', border: '1px solid rgba(0,200,255,.1)', borderRadius: '10px', padding: '11px 14px', fontSize: '14px', color: '#f0f8ff', outline: 'none' }} />
                </div>
                <div>
                  <label style={{ fontSize: '12px', color: '#6a8a95', fontWeight: 600, display: 'block', marginBottom: '6px' }}>REPETIR CONTRASEÑA</label>
                  <input type="password" required placeholder="••••••••" value={confirm} onChange={e => setConfirm(e.target.value)} style={{ width: '100%', background: '#111e25', border: '1px solid rgba(0,200,255,.1)', borderRadius: '10px', padding: '11px 14px', fontSize: '14px', color: '#f0f8ff', outline: 'none' }} />
                </div>

                ...
                <tbody>
                  {qrs.map((q: any) => (
                    <tr key={q.id} style={{ borderBottom: '1px solid rgba(255,255,255,.03)' }}>
                      ...
                      <td style={{ padding: "10px 8px", textAlign: "center" }}><button onClick={() => {fetch(`https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=https://qrnet.io/q/${q.public_code}`).then(r=>r.blob()).then(b=>{const u=URL.createObjectURL(b);const a=document.createElement("a");a.href=u;a.download=`QR-${q.public_code}.png`;a.click();URL.revokeObjectURL(u)})}} style={{background:"rgba(0,200,255,.15)",color:"#00c8ff",border:"none",padding:"4px 10px",borderRadius:"6px",fontSize:"11px",fontWeight:700,cursor:"pointer",marginLeft:"8px"}}>⬇ QR</button>
                      </td>

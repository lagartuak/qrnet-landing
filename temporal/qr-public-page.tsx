...
function pvrStatus(fecha: string | null) {
  ...
  return {
    estado: 'ok',
    texto: `Vigente · caduca ${new Date(fecha).toLocaleDateString('es-ES')}`,
    clase: 'pvr-ok'
  };
}
...
export default async function QRPublicPage({ params }: Props) {
  ...
  const [rows]: any = await pool.query(
    `SELECT qr_codes.*, users.name as owner_name, users.email as owner_email
     FROM qr_codes 
  ...
  // Registrar escaneo (sin await para no bloquear)
  pool.query(
    'INSERT INTO qr_scans (qr_code_id, scanned_at, ip) VALUES (?, NOW(), ?)',
    [qr.id, '0.0.0.0']
  ).catch(() => {});
  ...
  const esMaquinaTabaco = qr.object_type === 'maquina-tabaco' && data.tipo_maquina !== 'vending';
  ...
  return (
    <div className="qr-page">
      ...
      <div className="qr-card">
        ...
        <div className="qr-card-header">
          ...
          <div className="qr-addr">
            ...
            <span>
              {data.estab_dir}{data.estab_cp ? `, ${data.estab_cp}` : ''} {data.estab_ciudad}
              {data.estab_ciudad ? ' (Navarra)' : ''}
            </span>
          </div>
      ...
      <div className="qr-footer">
        ...
        <a href="https://www.maquinasdetabaco.com" target="_blank" rel="noreferrer">
          maquinasdetabaco.com
        </a>

    ...
    <main style={{ minHeight: '100vh', background: '#020608', color: '#c8dde5', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '40px 20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
          ...
          {qrCount < qrLimit && (
            <a href="/dashboard/new" style={{ background: '#00c8ff', color: '#000', padding: '12px 24px', borderRadius: '40px', fontWeight: 700, fontSize: '14px', textDecoration: 'none' }}>
              + Nuevo QR
            </a>
          )}
          ...
          {qrRows.map((qr: any) => (
            <div key={qr.id} style={{ background: '#0d1a20', borderRadius: '12px', padding: '24px', border: '1px solid rgba(0,200,255,.1)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                <h3 style={{ color: '#f0f8ff', fontSize: '16px', fontWeight: 600 }}>{qr.label || 'Sin nombre'}</h3>
                <span style={{ background: qr.active ? 'rgba(0,200,255,.1)' : 'rgba(255,107,53,.1)', color: qr.active ? '#00c8ff' : '#ff6b35', fontSize: '11px', padding: '4px 10px', borderRadius: '40px', fontWeight: 600 }}>
